...
anchor_lang::declare_program!(voting);

use self::voting::accounts::{PollAccount, CandidateAccount};
...
fn initialize_poll(
    ...
    let ix = ctx
        ...
        .args(args::InitPoll {
            _poll_id: poll_id,
            poll_name: name.to_string(),
            poll_description: description.to_string(),
            poll_voting_start: start_time,
...
#[test]
fn test_vote_invalid_candidate_fails() {
    let mut ctx = setup();
    let authority = ctx.svm.create_funded_account(10_000_000_000).unwrap();
    let voter = ctx.svm.create_funded_account(10_000_000_000).unwrap();
    ...
    initialize_poll(
        ...
        2_000,
        "Test Poll",
        "A failing voting test",
    );
    initialize_candidate(&mut ctx, &authority, poll_id, "Alice");
    
    cast_vote(&mut ctx, &voter, poll_id, "Bob")
         .assert_failure()
         .assert_anchor_error("AccountNotInitialized");
}

#[test]
fn test_vote_multiple_times() {
    let mut ctx = setup();
    let authority = ctx.svm.create_funded_account(10_000_000_000).unwrap();
    let voter1 = ctx.svm.create_funded_account(10_000_000_000).unwrap();
    let voter2 = ctx.svm.create_funded_account(10_000_000_000).unwrap();
    ...
    let result = cast_vote(&mut ctx, &voter1, poll_id, "Alice");
    result.assert_success();

    let result = cast_vote(&mut ctx, &voter2, poll_id, "Alice");
    result.assert_success();

    let alice_account: CandidateAccount = ctx.get_account(&get_candidate_pda(poll_id, "Alice")).unwrap();
    assert_eq!(alice_account.candidate_votes, 2);
}